...
public class CommandListClaimFlags implements TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (claim != null) {
            ...
            for (Flag flag : flags) {
                flagsFound = true;
                builder1.append(flag.getSet() ? "&a" : "&c").append(flag.getFlagDefinition().getName()).append(flag.parameters.length() > 0 ? "&7(" + flag.parameters + ")" : "").append(" ");
            }
            ...
            if (claim.parent != null) {
                ...
                for (Flag flag : flags) {
                    flagsFound = true;
                    builder2.append(flag.getSet() ? "&a" : "&c").append(flag.getFlagDefinition().getName()).append(flag.parameters.length() > 0 ? "&7(" + flag.parameters + ")" : "").append(" ");
                }
            }
            ...
            for (Flag flag2 : flags) {
                flagsFound = true;
                builder3.append(flag2.getSet() ? "&a" : "&c").append(flag2.getFlagDefinition().getName()).append(flag2.parameters.length() > 0 ? "&7(" + flag2.parameters + ")" : "").append(" ");
            }
        }
        ...
        for (Flag flag3 : flags) {
            flagsFound = true;
            builder4.append(flag3.getSet() ? "&a" : "&c").append(flag3.getFlagDefinition().getName()).append(flag3.parameters.length() > 0 ? "&7(" + flag3.parameters + ")" : "").append(" ");
        }
        ...
        for (Flag flag4 : flags) {
            flagsFound = true;
            builder5.append(flag4.getSet() ? "&a" : "&c").append(flag4.getFlagDefinition().getName()).append(flag4.parameters.length() > 0 ? "&7(" + flag4.parameters + ")" : "").append(" ");
        }
